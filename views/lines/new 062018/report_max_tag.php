...
<?php
	$sql_model="SELECT label_model_no FROM prod_fg_tag.fgt_model ORDER BY label_model_no ";
	$qr_model=mysql_query($sql_model);
	$total=mysql_num_rows($qr_model);
		$i=1;
		if($total<>0){	
	...
	DATE_FORMAT(a.date_print,'%d-%b-%Y %H:%i') AS dateprint ,a.tag_qty,
		...
		FROM prod_fg_tag.fgt_tag a 
		LEFT JOIN prod_fg_tag.fgt_serial b ON a.tag_no=b.tag_no 
		LEFT JOIN prod_fg_tag.fgt_model c ON a.id_model=c.id_model
