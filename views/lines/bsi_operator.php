...
<div class="body_resize"  align="center"> 
 ...
 <form id="form1" name="form1" method="post" action="index_bsi.php?id=<?=base64_encode('bsi_operator')?>" >
   <table width="50%" border="1" align="center" class="table01"  >
     <tr>
       <td width="225" height="37"><div class="tmagin_right">Search : Work Date, Line Name</div> </td>
       <td width="323">
    ...
    <tr>
      ...
      <th>Add</th>
       ...
       <?php while($rs=mysql_fetch_array($qr)){  
			...
			FROM ".DB_DATABASE1.".fgt_leader b 
			LEFT JOIN ".DB_DATABASESSO.".so_employee_data d ON d.emp_id=b.leader_day
			LEFT JOIN ".DB_DATABASESSO.".so_employee_data e ON e.emp_id=b.leader_night
			LEFT JOIN ".DB_DATABASESSO.".so_employee_data f ON f.emp_id=b.floater_day
			LEFT JOIN ".DB_DATABASESSO.".so_employee_data g ON g.emp_id=b.floater_night
      ...
      <td height="30px" <?php if($np>1){ $nps = $np+1;echo "rowspan='".$nps."'";}?> > <?php echo $rs['wdate'];?></td>
      ...
      <?php
		 ...
		 ?>
          ...
          <td > 
           <? if($ssumo<>'0'){?>
          <a href="index_bsi.php?id=<?=base64_encode('bsi_setup_operetor_emp')?>&wid=<?=$rsp['work_id'];?>&st=edit"  >
            <img src="../../images/001_45.gif" alt="Update" /><? $rsp['work_id'];?></a>
            <?php }?>
          </td>
          <td>  <a href="#" onClick="javascript:openWins('windows.php?win=view_opb&idm=<?=$rsp['work_id']?>', '_blank',1100,750,1,1, 0, 0, 0);return false;" >
              
              <img src="../../images/view2.png" width="24" height="24"  border="0" alt="View"/></a> 
        </td>
          <td><a href="index.php?id=<?=base64_encode('bsi_setup_operetor_emp')?>&wid=<?=$rsp['work_id'];?>&st=add"  >
        ...
        <?php
		 ...
		 ?>
          ...
          <td><a href="index_bsi.php?id=<?=base64_encode('bsi_setup_operetor_emp')?>&wid=<?=$rsp['work_id'];?>&st=add"  >
        <img src="../../images/001_01.gif" alt="Add"/><?php $rsp['work_id'];?></a>
        </td>
            </tr>
             <?php
		 	}//	while($rsp=mysql_fetch_array($qrp)){
...
<div class="browse_page" >
			  <?php       @page_navigator_user($before_p,$plus_p,$total,$total_p,$chk_page,base64_encode('bsi_operator'),$txtsearsh);  	  ?>
  </div>  
<?php }
