	...
	if(!empty($_POST['button']) && $_POST['button']=="Upload"){
		...
		$today=date('Y-m-d H:i:s');
			...
			while($rsck=mysql_fetch_array($qrck)){ 
				...
				'".$rsck['date_reprint']."','".$top_name."','".$today."')
				...
				shift='".$rsck['shift']."',	sn_start='".$rsck['sn_start']."', sn_end='".$rsck['sn_end']."', tag_qty='".$rsck['tag_qty']."',
					...
					date_print='".$rsck['date_print']."', who_reprint='".$rsck['who_reprint']."', date_reprint='".$rsck['date_reprint']."',
					 who_upload='".$top_name."', date_upload='".$today."'";
				...
				mysql_query("INSERT INTO ".DB_DATABASE1.".fgt_update_tag SET last_tag_id='$maxtag', line_id='$user_login',
						  status_update='$top_name', date_update='".$today."'",$con);
				 mysql_query("INSERT INTO ".DB_DATABASE4.".fgt_log_local_update_tag SET last_tag_id='$maxtag', line_id='$user_login',
						  status_update='$top_name', date_update='".$today."'",$con2);
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
 ...
 <div class="rightPane" align="center">
  <?
		...
		$qrup=mysql_query($sqlup,$con);
		$numup=mysql_num_rows($qrup);  
		if($numup<>0){
			$i=1;
 ...
 <table width="663px" border="1" class="table01" >
   ...
   <tr align="center">
     ...
     <td><?
			...
			$qrdt=mysql_query($sqldt,$con);
			$numup=mysql_num_rows($qrdt);  
			if($numup<>0){
 ...
 ?>
  ...
  <table width="98%" border="1" bordercolor="#CC9966"class="table01" align="center">
    ...
    <tr>
      ...
      <td><?=$rs['tag_qty']?></td>
      <td><?=$rs['dateprint']?></td>
      <td><? echo $rs['datereprint']." ".$rs['whoreprint']; ?></td>
    </tr>
	 <? 
        $i++;
