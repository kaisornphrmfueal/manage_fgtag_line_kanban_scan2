...
include('../../includes/configure_srv.php');
	//	Ccheck Model, Line, Bartender ,Pattern
	
	if(!empty($_GET['idup']) && $_GET['idup']=="Pattern"){
		//alert($_GET['idup']);	
			$gtype= $_GET['idup']; //Pattern
			...
			$mxsvup = $rsmxs['mxsidrvup'];
		 	...
		 	$sqlup="SELECT b.* 
				...
				$sqlq="SELECT id,pattern_serial,patmonth,patyear FROM ".DB_DATABASE4.".fgt_pattern_serial WHERE id = '".$rsup['id_action']."' ";	
				$qrq=mysqli_query($con, $sqlq);		
						while($rsq=mysqli_fetch_array($qrq)){
						$uppatternid=$rsq['id'];
						$pattern=$rsq['pattern_serial'];
						$patmonth=$rsq['patmonth'];
						$patyear=$rsq['patyear'];
						
						if($actionn=="Insert"){
							$sqlup="INSERT INTO ".DB_DATABASE1.".view_pattern SET id = '$uppatternid',
							pattern_serial = '$pattern',patmonth = '$patmonth',patyear = '$patyear',
							emp_modify = '$user_login',datetime_modify = '$timenow' ";
							mysqli_query($con, $sqlup);
						}//if($actionn=="Insert"){
							else{
								//Update
								$sqlup="UPDATE ".DB_DATABASE1.".view_line SET .view_pattern SET 
								pattern_serial = '$pattern',patmonth = '$patmonth',patyear = '$patyear',
								emp_modify = '$user_login',datetime_modify = '$timenow'
								WHERE id = '$uppatternid' ";
								mysqli_query($con, $sqlup);
								
								}//else{
							}//while($rsq=mysql_fetch_array($qrq)){
	...
	if(!empty($_GET['idup']) && $_GET['idup']=="Model"){
					...
					if(mysqli_num_rows($qrq)<>0){
						...
						if($actionn=="Insert"){
								 ...
								 tag_model_no='".$rsq['tag_model_no']."', label_model_no='".$rsq['label_model_no']."', model_name='".$rsq['model_name']."', 
									...
									customer_part_name='".$rsq['customer_part_name']."', model_picture='".$modelpic."',status_tag_printing='".$rsq['status_tag_printing']."',
									emp_insert='$user_login', date_insert='".$timenow."'";
								mysqli_query($con, $sqlqrup);
		...
		if(!empty($_GET['idup']) && $_GET['idup']=="Bartender"){
			...
			$mxsvup = $rsmxs['mxsidrvup'];
				...
				$sqlupl="INSERT INTO ".DB_DATABASE1.".fgt_update_master SET last_id_srvupdate='$mxsvup', 
						type_data='$gtype', user_update='$user_login', date_update='$timenow'";
				mysqli_query($con, $sqlupl);//Update local
				//End  Update local
				//Start Update Server Log
...
<?php
			...
			$arr = array("Model", "Line", "Bartender","Permission","Pattern");
			$i=0;
...
(การจัดการข้อมูลจากระบบหลัก)</div> </th>
    ...
    <tr>
      ...
      <th width="8%">Update<br />อัพเดต</th>
      ...
      <th width="15%">User modified<br />ผู้แก้ไข</th>
    </tr>
       <?php while ($i < 5) {   
	   		$rstype=$arr[$i];
