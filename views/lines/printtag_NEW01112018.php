<?
	...
	if(!empty($_POST['button2']) && $_POST['button2']=="Submit" && !empty($_POST['htid']) ){ 
		...
		//--------- Check pattern serial ----------------
			...
			$sql_pat="SELECT pattern_serial FROM ".DB_DATABASE1.".view_pattern WHERE pattern_serial = '$txtserial' ";
              ...
              <tr>
                ...
                <?
                 	  $sql_sr="SELECT b.id_serial, b.serial_scan_label  
								...
								AND CONCAT(MONTH(b.date_scan),"",YEAR(b.date_scan)) = CONCAT(MONTH(NOW()),"",YEAR(NOW()))
