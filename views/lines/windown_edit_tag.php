...
if(!empty($_POST['hbutton'])){
	...
	//	$pidtag=$_POST['hidtag'];
		...
		for($k=1;$k<=$pqtyp;$k++){
			...
			$sqlup="UPDATE ".DB_DATABASE1.".fgt_srv_serial 
			SET serial_scan_label = '$pserial',
			emp_modify = '$emp_confirm',
			date_modify = '".date('Y-m-d H:i:s')."' 
			WHERE id_serial='$pidserial' ";
			$qrup=mysqli_query($con, $sqlup);
			log_hist($emp_confirm,"Adjustment Serial",$pidserial,"fgt_srv_serial",$sqlup);
			//if($qrup){
				
				if($k==$pqtyp){
					//updat sn_start in fgt_srv_tag
					//alert($k."=".$pqtyp);
					$sqlup_st="UPDATE ".DB_DATABASE1. ".fgt_srv_tag 
					SET sn_start = '".$_POST['edit_1']."',sn_end = '".$_POST['edit_'.$pqtyp]."'
					WHERE tag_no='$ptagn' ";
					$qrup_st=mysqli_query($con, $sqlup_st);
					log_hist($emp_confirm,"Adjustment Tag sn_start,sn_end",$ptagn, "fgt_srv_tag",$sqlup_st);
						}
					//}//if($qrup){
				}//if(!empty($pserial) AND !empty($pidserial)){
				else{
...
<table width="623"  border="1" class="table01" align="center">
  <tr>
   ...
   <tr>
     ...
     <p class="text_black_bold"> Model No. (หมายเลขโมเดล) :  <?=$tmodel?>   </p>
      ...
      <p class="text_black_bold">Serial No. (หมายเลขซีเรียล) :   <?php 
	 	$sqls="SELECT CASE  $tqty WHEN 1 THEN  MIN(serial_scan_label) ELSE   CONCAT(MIN(serial_scan_label) ,'-' ,MAX(serial_scan_label)) END AS serialp
				FROM prod_fg_tag.fgt_srv_serial WHERE tag_no = '$tno'";
	...
	<!-- <tr  align="center">
    <?php //for($i=1;$i<=$sqty;$i++){?> 
      <td height="61" bgcolor="#66CCCC"><span class="text_black">Serial <?=$i;?></span></td>
    <?php //}?>
    </tr>-->
    <?php //for($i=1;$i<=$sqty;$i++){
		$sql="SELECT serial_scan_label,id_serial FROM prod_fg_tag.fgt_srv_serial WHERE tag_no = '$tno' ";
		$qr=mysqli_query($con, $sql);
		$j=1;
		while($re=mysqli_fetch_array($qr)){	
	?> 
    <tr>
     <td height="61" bgcolor="#66CCCC" align="center"><span class="text_black">Serial <?=$j;?></span></td>
    </tr>
    <tr align="center">
      ...
      <?php $j++;}//while($re=mysql_fetch_array()){
	  //} ?>
