	...
	if(!empty($_POST['button2']) && $_POST['button2']=="Submit" && !empty($_POST['htagno']) ){ 
		...
		$phtagbc=$_POST['htagbc']; 
	 	 ...
	 	 $sqlct="SELECT a.tag_no,c.tag_model_no,c.label_model_no,a.line_id,a.fg_tag_barcode ,
				b.serial_scan_label, COUNT(b.tag_no)+1 AS sr_count,c.std_qty,c.model_destination
				...
				//echo "numct==0";
					...
					if($rsckn['std_qty']==1){
						...
						$sqlup_tag="UPDATE ".DB_DATABASE1.".fgt_srv_tag SET  sn_start='$psserial' , sn_end='$psserial' ,
							...
							log_hist($emp_confirm,"Insert Adjust",$psserial,"fgt_srv_serial",$sqlsr);
						...
						// END  INSERT FOR MATCHING 
						 ...
						 printTag($ptagno);
						//eprinTag
							log_hist($emp_confirm,"Printed Tag Adjust",$ptagno,"fgt_srv_tag","");
...
function validateTagAdjust(my_request) {
...
<form name="scan" id="scan" method="post" action="index.php?id=<?=base64_encode('adjust_serial_scan')?>" onsubmit="return false;"  autocomplete="off" >
              ...
              <tr>
				...
				<?php
				 ...
				 echo $top_name." Real Time Printing (Adjustment Data)";
				 ?>
                </span>  </th>
              </tr>
              <tr>
                ...
                <span class="text_black"><?php  echo "<span id=txtModel>$stmodel</span> ";?> </span>
                <span class="txt-blue-big"><?php echo "[".$rsst['model_name']."]"; ?></span>
            ...
            onclick="window.location.href='index.php?id=<?=base64_encode('print')?>&wtag=<?=$sttag?>';"/>-->
              ...
              <tr>
                ...
                <?php
					...
					$qr_sr=mysqli_query($con, $sql_sr);
						if($num_st=mysqli_num_rows($qr_sr)<>0){
							$rssr=mysqli_fetch_array($qr_sr);
               ...
               <span class="txt-black-big"> 	<?php  echo $stticket_qty; ?> </span> </div>
                ...
                <div class="tmagin_right">
                  ...
                  <input type="button" name="btnscan" id="btnscan" value="Submit" class="myButton" onclick="validateTagAdjust(this.value)" />
...
<!--</form>!-->
	...
	<a href='index.php?id=".base64_encode('adjust_model_scan')."'>ไม่มีข้อมูลการตั้งต้นของโมเดล คลิกที่นี่ เพื่อแสกน Kanban อีกครั้ง</a></div></center>";
