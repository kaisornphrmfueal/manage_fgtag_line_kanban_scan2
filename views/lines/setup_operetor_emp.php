...
if($_POST['button']=="Submit"){
			...
			//echo "--->".$_POST['hidLinen_0067']."--->".$_POST['pro_work_0067'];
					...
					foreach( $idemp as $emps ):
							...
							//echo $emps;
							 ...
							 //$j++;
							 $sqlrw="SELECT * FROM ".DB_DATABASE1.".fgt_operator WHERE work_id = '$pwork_id' AND operater_id = '$idemp'";
								$numrw=mysqli_num_rows(mysqli_query($con, $sqlrw));
							 if($numrw=='0'){
	...
	if($qr_del){
		...
		gotopage("index.php?id=".base64_encode('setup_operetor_emp')."&wid=".$_GET['wid']."&st=".$_GET['st']);
		}
...
IF(LOWER(a.shift)='day',CONCAT(f.emp_id,' ',f.name_en,' ',f.lastname_en),CONCAT(g.emp_id,' ',g.name_en,' ',g.lastname_en))AS sname_floater  
					...
					LEFT JOIN ".DB_DATABASE5.".so_employee_data d ON d.emp_id=a.leader_day
					LEFT JOIN ".DB_DATABASE5.".so_employee_data e ON e.emp_id=a.leader_night
					LEFT JOIN ".DB_DATABASE5.".so_employee_data f ON f.emp_id=a.floater_day
					LEFT JOIN ".DB_DATABASE5.".so_employee_data g ON g.emp_id=a.floater_night
...
<table width="642" border="1" class="table01" align="center">
          <tr>
...
<center>
  ...
  <table width="90%" height="55" border="1" class="table01" align="center">
    ...
    <tr>
      ...
      <th width="8%" align="center">Type Shift</th>
      <th width="23%" align="center">Process Work</th>
    ...
    <?php $v =0; while($rs=mysqli_fetch_array($qr)){ 	?>
       ...
       <select name="pro_work1[]<?=$i?>"  id="pro_work1"   style="width:100%"   >
		...
		<input type="hidden" name="emp[]<?=$i?>" id="emp" value="<?=$rs['emp_id']?>" />
         ...
         <td><a href="index.php?id=<?=base64_encode('setup_operetor_emp')?>&wid=<?=$rsm['work_id']?>&del=<?=$rs['emp_id']?>&st=<?=$_GET['st']?>" 
         onclick="return confirm('Are you sure you want to Delete?')"><img src="../../images/001_29.gif" width="25" height="25" border="0"/></a></td>
